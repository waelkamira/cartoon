...
export default function VideoPlayer({
  ...
  const blockAdsAndPopups = () => {
    ...
    // إزالة العناصر المتعلقة بالإعلانات
    const removeAds = () => {
      const adSelectors = [
        'iframe[src*="ads"]',
        'div[class*="ad"]',
    ...
    // راقب التعديلات الديناميكية على الصفحة
  ...
  const sanitizeIframe = (iframe) => {
    ...
    const adSelectors = ['iframe[src*="ads"]', 'div[class*="ad"]'];
    adSelectors.forEach((selector) => {
  ...
  useEffect(() => {
    ...
    iframes.forEach((iframe) => sanitizeIframe(iframe));
  }, []);
